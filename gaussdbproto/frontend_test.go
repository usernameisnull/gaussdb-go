...
func TestFrontendReceiveInterrupted(t *testing.T) {
	...
	frontend := gaussdbproto.NewFrontend(server, nil)
	...
	if msg, ok := msg.(*gaussdbproto.ReadyForQuery); !ok || msg.TxStatus != 'I' {
...
func TestErrorResponse(t *testing.T) {
	...
	want := &gaussdbproto.ErrorResponse{
		Severity:            "ERROR",
...
func TestFrontendReceiveExceededMaxBodyLen(t *testing.T) {
	...
	frontend := gaussdbproto.NewFrontend(client, nil)
	...
	msg, err := frontend.Receive()
	assert.Nil(t, msg)
	var invalidBodyLenErr *gaussdbproto.ExceededMaxBodyLenErr
