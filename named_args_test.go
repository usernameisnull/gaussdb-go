...
func TestNamedArgsRewriteQuery(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		},
		{
			...
			namedArgs:    gaussdbgo.NamedArgs{"extra": int32(1)},
...
func TestStrictNamedArgsRewriteQuery(t *testing.T) {
	...
	for i, tt := range []struct {
		sql             string
		namedArgs       gaussdbgo.StrictNamedArgs
		expectedSQL     string
		expectedArgs    []any
		...
		{
			sql:             "@all @matches",
			namedArgs:       gaussdbgo.StrictNamedArgs{"all": int32(1), "matches": int32(2)},
			expectedSQL:     "$1 $2",
		...
		{
			sql:             "extra provided argument",
			namedArgs:       gaussdbgo.StrictNamedArgs{"extra": int32(1)},
