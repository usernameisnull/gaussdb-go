...
func TestArrayCodecFlatArrayString(t *testing.T) {
	...
	queryModes := []gaussdbx.QueryExecMode{gaussdbx.QueryExecModeSimpleProtocol, gaussdbx.QueryExecModeDescribeExec}
...
func TestArrayCodecArray(t *testing.T) {
	...
	ctr.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *gaussdbx.Conn) {
		for i, tt := range []struct {
			expected any
		}{
			{gaussdbtype.Array[int32]{
				Elements: []int32{1, 2, 3, 4},
				Dims: []gaussdbtype.ArrayDimension{
			...
			var actual gaussdbtype.Array[int32]
			err := conn.QueryRow(
				ctx,
				"select $1::int[]",
				tt.expected,
...
func TestArrayCodecSliceArgConversion(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *gaussdbx.Conn) {
		...
		var expected []gaussdbtype.UUID

		for _, s := range arg {
			buf, err := hex.DecodeString(strings.ReplaceAll(s, "-", ""))
			require.NoError(t, err)
			var u gaussdbtype.UUID
			copy(u.Bytes[:], buf)
			u.Valid = true
			expected = append(expected, u)
		}

		var actual []gaussdbtype.UUID
		err := conn.QueryRow(
			ctx,
			"select $1::uuid[]",
			arg,
