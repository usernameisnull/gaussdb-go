...
func TestHstoreCodec(t *testing.T) {
	...
	ctr.AfterConnect = func(ctx context.Context, t testing.TB, conn *gaussdbgo.Conn) {
		...
		conn.TypeMap().RegisterType(&gaussdbtype.Type{Name: "hstore", OID: hstoreOID, Codec: gaussdbtype.HstoreCodec{}})
	}

	tests := []gaussdbxtest.ValueRoundTripTest{
		{
			map[string]string{},
		...
		{nil, new(*map[string]*string), isExpectedEq((*map[string]*string)(nil))},
		{nil, new(*gaussdbtype.Hstore), isExpectedEq((*gaussdbtype.Hstore)(nil))},
	}
	...
	for _, s := range specialStrings {
		...
		// at beginning
		tests = append(tests, gaussdbxtest.ValueRoundTripTest{
	...
	for i := range tests {
		...
		case map[string]*string:
			h = gaussdbtype.Hstore(typedParam)
		case map[string]string:
			if typedParam != nil {
				h = gaussdbtype.Hstore{}
		...
		tests[i].Param = h
		tests[i].Result = &gaussdbtype.Hstore{}
	...
	gaussdbxtest.RunWithQueryExecModes(ctx, t, ctr, gaussdbxtest.AllQueryExecModes, func(ctx context.Context, t testing.TB, conn *gaussdbgo.Conn) {
		h := gaussdbtype.Hstore{"should_be_erased": nil}
		err := conn.QueryRow(ctx, `select cast(null as hstore)`).Scan(&h)
		if err != nil {
			t.Fatal(err)
		}
		expectedNil := gaussdbtype.Hstore(nil)
...
func TestRoundTrip(t *testing.T) {
	codecs := []struct {
		name       string
		encodePlan gaussdbtype.EncodePlan
		scanPlan   gaussdbtype.ScanPlan
	}{
		{
			"text",
			gaussdbtype.HstoreCodec{}.PlanEncode(nil, 0, gaussdbtype.TextFormatCode, gaussdbtype.Hstore(nil)),
			gaussdbtype.HstoreCodec{}.PlanScan(nil, 0, gaussdbtype.TextFormatCode, (*gaussdbtype.Hstore)(nil)),
		},
		{
			"binary",
			gaussdbtype.HstoreCodec{}.PlanEncode(nil, 0, gaussdbtype.BinaryFormatCode, gaussdbtype.Hstore(nil)),
			gaussdbtype.HstoreCodec{}.PlanScan(nil, 0, gaussdbtype.BinaryFormatCode, (*gaussdbtype.Hstore)(nil)),
		},
	}

	inputs := []gaussdbtype.Hstore{
		nil,
		{},
		{"": stringPtr("")},
		{"k1": stringPtr("v1")},
...
func BenchmarkHstoreEncode(b *testing.B) {
	h := gaussdbtype.Hstore{"a x": stringPtr("100"), "b": stringPtr("200"), "c": stringPtr("300"),
	...
	serializeConfigs := []struct {
		...
		{"text", gaussdbtype.HstoreCodec{}.PlanEncode(nil, 0, gaussdbtype.TextFormatCode, h)},
		{"binary", gaussdbtype.HstoreCodec{}.PlanEncode(nil, 0, gaussdbtype.BinaryFormatCode, h)},
	}
...
func BenchmarkHstoreScan(b *testing.B) {
	...
	codec := gaussdbtype.HstoreCodec{}.PlanEncode(nil, 0, gaussdbtype.BinaryFormatCode, gaussdbtype.Hstore(nil))
	for i, s := range benchStrings {
		textBytes[i] = []byte(s)

		var tempH gaussdbtype.Hstore
		err := tempH.Scan(s)
		if err != nil {
			b.Fatal(err)
		}
	...
	scanConfigs := []struct {
		...
		{"text", gaussdbtype.HstoreCodec{}.PlanScan(nil, 0, gaussdbtype.TextFormatCode, &h), textBytes},
		{"binary", gaussdbtype.HstoreCodec{}.PlanScan(nil, 0, gaussdbtype.BinaryFormatCode, &h), binaryBytes},
	}
