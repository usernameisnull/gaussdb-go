...
func TestParseConfigExtractsPoolArguments(t *testing.T) {
	...
	config, err := gaussdbxpool.ParseConfig("pool_max_conns=42 pool_min_conns=1")
...
func TestConnectConfigRequiresConnConfigFromParseConfig(t *testing.T) {
	...
	config := &gaussdbxpool.Config{}

	require.PanicsWithValue(t, "config must be created by ParseConfig", func() { gaussdbxpool.NewWithConfig(context.Background(), config) })
}
...
func TestConfigCopyCanBeUsedToConnect(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		_, err = gaussdbxpool.NewWithConfig(context.Background(), copied)
	})
	assert.NoError(t, err)
}
...
func TestPoolAcquireFunc(t *testing.T) {
	...
	var n int32
	err = pool.AcquireFunc(ctx, func(c *gaussdbxpool.Conn) error {
...
func TestPoolBeforeAcquire(t *testing.T) {
	...
	conns := make([]*gaussdbxpool.Conn, 4)
	for i := range conns {
...
func TestPoolAcquireAllIdle(t *testing.T) {
	...
	conns := make([]*gaussdbxpool.Conn, 3)
	for i := range conns {
		conns[i], err = db.Acquire(ctx)
		assert.NoError(t, err)
	}
