...
func (cr *connResource) getPoolRow(c *Conn, r gaussdbgo.Row) *poolRow {
...
func (cr *connResource) getPoolRows(c *Conn, r gaussdbgo.Rows) *poolRows {
...
type Pool struct {
	...
	config                *Config
	beforeConnect         func(context.Context, *gaussdbgo.ConnConfig) error
	afterConnect          func(context.Context, *gaussdbgo.Conn) error
	beforeAcquire         func(context.Context, *gaussdbgo.Conn) bool
	afterRelease          func(*gaussdbgo.Conn) bool
	beforeClose           func(*gaussdbgo.Conn)
	minConns              int32
	maxConns              int32
...
type Config struct {
	...
	// BeforeConnect is called before a new connection is made. It is passed a copy of the underlying gaussdbgo.ConnConfig and
	...
	p.p, err = puddle.NewPool(
		&puddle.Config[*connResource]{
			...
			Destructor: func(value *connResource) {
				...
				conn.Close(ctx)
				select {
				case <-conn.GaussdbConn().CleanupDone():
				case <-ctx.Done():
				}
				cancel()
			},
...
// ParseConfig builds a Config from connString. It parses connString with the same behavior as [gaussdbgo.ParseConfig] with the
...
func (p *Pool) Exec(ctx context.Context, sql string, arguments ...any) (gaussdbconn.CommandTag, error) {
	c, err := p.Acquire(ctx)
	if err != nil {
...
// Query acquires a connection and executes a query that returns gaussdbgo.Rows.
// Arguments should be referenced positionally from the SQL string as $1, $2, etc.
// See gaussdbgo.Rows documentation to close the returned Rows and return the acquired connection to the Pool.
//
// If there is an error, the returned gaussdbgo.Rows will be returned in an error state.
// If preferred, ignore the error returned from Query and handle errors using the returned gaussdbgo.Rows.
//
// For extra control over how the query is executed, the types QuerySimpleProtocol, QueryResultFormats, and
// QueryResultFormatsByOID may be used as the first args to control exactly how the query is executed. This is rarely
// needed. See the documentation for those types for details.
func (p *Pool) Query(ctx context.Context, sql string, args ...any) (gaussdbgo.Rows, error) {
	c, err := p.Acquire(ctx)
	if err != nil {
...
// to return at most one row (gaussdbgo.Row). Errors are deferred until gaussdbgo.Row's
// Scan method is called. If the query selects no rows, gaussdbgo.Row's Scan will
// return ErrNoRows. Otherwise, gaussdbgo.Row's Scan scans the first selected row
// and discards the rest. The acquired connection is returned to the Pool when
// gaussdbgo.Row's Scan method is called.
//
// Arguments should be referenced positionally from the SQL string as $1, $2, etc.
//
// For extra control over how the query is executed, the types QuerySimpleProtocol, QueryResultFormats, and
// QueryResultFormatsByOID may be used as the first args to control exactly how the query is executed. This is rarely
// needed. See the documentation for those types for details.
func (p *Pool) QueryRow(ctx context.Context, sql string, args ...any) gaussdbgo.Row {
	c, err := p.Acquire(ctx)
	if err != nil {
...
func (p *Pool) SendBatch(ctx context.Context, b *gaussdbgo.Batch) gaussdbgo.BatchResults {
	c, err := p.Acquire(ctx)
	if err != nil {
...
func (p *Pool) CopyFrom(ctx context.Context, tableName gaussdbgo.Identifier, columnNames []string, rowSrc gaussdbgo.CopyFromSource) (int64, error) {
	c, err := p.Acquire(ctx)
	if err != nil {
		return 0, err
	}
